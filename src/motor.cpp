...
elmo::Motor::Motor(InputData_t * in, OutputData_t * out):
	in_(in), out_(out), drive_state_(DriveState::NA), enabled_(false)
{
...
static const int max_torque = 600;
...
// 如果超过限制，则返回真
bool elmo::Motor::encoder_check()
{
    bool end_program = true;
    if(this->getActualPosition() < 0 || this->getActualPosition() > 1638400 / 3 * 2)
    {
        this->enable(false);
    }
    else
